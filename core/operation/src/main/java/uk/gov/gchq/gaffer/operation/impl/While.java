...
/**
 * A {@code While} is an {@link Operation} which executes a provided delegate
 * Operation, either while some condition is true, (upto some global maximum),
 * or until some configurable cut-off is reached.
 * <p>The condition can either be in the form of anything that resolves to a
 * boolean, or a {@link Conditional} can be provided, to control the operation
 * based upon the input object itself. This allows the input to be transformed
 * non-destructively, before it is passed to the {@link Predicate} contained
 * within the Conditional.</p> <p>N.B. Only one of the condition and conditional
 * can be set - attempting to configure both will throw an exception.</p> <p>By
 * default, the operation is configured with a large number of repeats, which
 * will need altering using {@link While#setMaxRepeats(int)}.</p>
 */
@Since("1.5.0")
@Summary("Repeatedly executes an operation while a condition is met")
@JsonPropertyOrder(value = {"class", "input", "conditional", "operation",
                            "maxRepeats", "options"},
                   alphabetic = true)
@JsonInclude(JsonInclude.Include.NON_DEFAULT)
public class While<I, O> extends GenericInput<I>
  ...
  public void updateOperations(final Collection<Operation> operations) {
    if (null == operations || 2 != operations.size()) {
      throw new IllegalArgumentException(
          "Unable to update operations - exactly 2 operations are required. " +
          "Received " +
  ...
  public void setOperation(final Operation op) { this.operation = op; }
  ...
  public static final class Builder<I, O>
    ...
    public Builder() { super(new While<>()); }
    ...
    public Builder<I, O> condition(final Boolean condition) {
      if (null != condition && null != _getOp().getConditional()) {
        throw new IllegalArgumentException(
            "Tried to set condition when conditional has already been " +
            "configured.");
      }
    ...
    public Builder<I, O> conditional(final Conditional conditional) {
      if (null != conditional && null != _getOp().isCondition()) {
        throw new IllegalArgumentException(
            "Tried to set conditional when condition has already been " +
            "configured.");
      }
