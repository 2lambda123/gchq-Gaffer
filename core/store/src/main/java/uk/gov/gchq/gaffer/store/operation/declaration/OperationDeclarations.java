 ...
 * store.properties. Used to add operation handlers.
 */
public class OperationDeclarations {
  private List<OperationDeclaration> operations;

  public List<OperationDeclaration> getOperations() { return operations; }
  ...
  public static class Builder {
    ...
    public OperationDeclarations build() { return this.instance; }
  }

  public static OperationDeclarations fromPaths(final String paths) {
    final OperationDeclarations allDefinitions =
        new OperationDeclarations.Builder().build();
    ...
    try {
      for (final String pathStr : paths.split(",")) {
        ...
        if (path.toFile().exists()) {
          ...
          definitions = fromJson(
              StreamUtil.openStream(OperationDeclarations.class, pathStr));
        }
      ...
      throw new GafferRuntimeException(
          "Failed to load element definitions from paths: " + paths +
              ". Due to " + e.getMessage(),
          e);
    }
  ...
  public static OperationDeclarations fromJson(final byte[] json) {
    try {
      ...
      throw new GafferRuntimeException(
          "Failed to load element definitions from bytes. Due to " +
              e.getMessage(),
          e);
    }
  }
  ...
  public static OperationDeclarations fromJson(final String json) {
    try {
      ...
      throw new GafferRuntimeException(
          "Failed to load element definitions from JSON String. Due to " +
              e.getMessage(),
          e);
    }
  }
  ...
  public static OperationDeclarations fromJson(final InputStream inputStream) {
    try {
      return JSONSerialiser.deserialise(inputStream,
                                        OperationDeclarations.class);
    } catch (final SerialisationException e) {
      throw new GafferRuntimeException(
          "Failed to load element definitions from bytes. Due to " +
              e.getMessage(),
