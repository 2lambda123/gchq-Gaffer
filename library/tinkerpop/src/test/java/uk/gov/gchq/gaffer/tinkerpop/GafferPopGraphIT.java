...
public class GafferPopGraphIT {
    private static final MapStoreProperties MAP_STORE_PROPERTIES = 
		MapStoreProperties.loadStoreProperties("/tinkerpop/map-store.properties");
    private static GraphTraversalSource g;

    @BeforeAll
    public static void resetGraph() {
        GafferPopGraph gafferPopGraph = 
			GafferPopModernTestUtils.createModernGraph(GafferPopHasStepIT.class, MAP_STORE_PROPERTIES, MODERN_CONFIGURATION);
    ...
    public void shouldGetAllEdges() {
        ...
        assertThat(result)
			.extracting(r -> r.id())
			.containsExactlyInAnyOrder(MARKO.knows(JOSH),
										MARKO.knows(VADAS),
										MARKO.created(LOP),
										JOSH.created(LOP),
										JOSH.created(RIPPLE),
    ...
    public void shouldTruncateGetAllEdges() {
        ...
        assertThat(result)
			.extracting(r -> r.id())
			.hasSize(2)
			.containsAnyOf(MARKO.knows(JOSH),
							MARKO.knows(VADAS),
							MARKO.created(LOP),
							JOSH.created(LOP),
							JOSH.created(RIPPLE),
