...
public class Queries {
  ...
  private void runFullExample(final Graph graph, final User user)
    ...
    final OperationChain<Iterable<String>> opChain =
        input("South West")
            ...
            .then(getEntities("JunctionUse")
                      .view(createView()
                                .filter("startDate",
                                        inDateRange("2000/01/01", "2001/01/01"))
                                .summarise()
                                .filter("countByVehicleType",
                                        predicateMap().key("BUS").predicate(
                                            isMoreThan().value(1000L)))
                                .transform("countByVehicleType",
                                           freqMapExtractor().key("BUS"),
                                           "busCount")))
            .then(sort()
                      .groups("JunctionUse")
                      .property("busCount")
                      ...
                      .limit(2)
                      .deduplicate())
            .then(toCsv().vertex("Junction").property("busCount", "Bus Count"));
  ...
  private Graph createGraph(final User user)
      throws IOException, OperationException {
    final Graph graph =
        new Builder()
            .config(new GraphConfig.Builder().graphId("roadTraffic").build())
            .addSchemas(StreamUtil.openStreams(ElementGroup.class, "schema"))
            .storeProperties(
                StreamUtil.openStream(getClass(), "accumulo/store.properties"))
            .build();

    final OperationChain<Void> populateChain =
        new OperationChain.Builder()
            .first(new GenerateElements.Builder<String>()
                       .input(IOUtils.readLines(StreamUtil.openStream(
                           getClass(), "roadTrafficSampleData.csv")))
                       .generator(new RoadTrafficStringElementGenerator())
                       .build())
            .then(new AddElements.Builder().skipInvalidElements(false).build())
            .build();
